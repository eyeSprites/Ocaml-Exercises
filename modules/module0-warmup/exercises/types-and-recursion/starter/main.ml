...
let rec string_of_expr (e : expr) : string =
  ...
  | Var name -> name
  | BinOp (op, left, right) ->
      "(" ^ string_of_expr left ^ " " ^ string_of_op op ^ " " ^ string_of_expr right ^ ")"
...
let rec substitute (var_name : string) (value : int) (e : expr) : expr =
  match e with
  | Num n -> Num n
  | Var name -> if name = var_name then Num value else Var name
  | BinOp (op, left, right) ->
      BinOp (op, substitute var_name value left, substitute var_name value right)
